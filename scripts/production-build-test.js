...
function analyzeBundleSize() {
  ...
  try {
    ...
    if (fs.existsSync(distPath)) {
      ...
      console.log(`📦 Web bundle directory size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);
      ...
      if (largeFiles.length > 0) {
        ...
        testResults.warnings.push('Large bundle files detected - consider code splitting');
      }
      
      // Bundle size recommendations
      const totalSizeMB = parseFloat(testResults.bundleSize.total);
      if (totalSizeMB > 10) {
        testResults.warnings.push('Bundle size exceeds 10MB - consider optimization');
      } else if (totalSizeMB > 5) {
        testResults.warnings.push('Bundle size exceeds 5MB - monitor performance');
      } else {
...
function checkTypeScript() {
  ...
  if (result.success) {
    testResults.optimizations.push('TypeScript compilation successful - no type errors');
    return true;
  } else {
    testResults.errors.push({
...
function runLinting() {
  ...
  try {
    ...
    const result = runCommand('npx eslint . --ext .ts,.tsx --max-warnings 0', 'ESLint check');
    if (result.success) {
      testResults.optimizations.push('Code quality checks passed');
    }
  } catch (error) {
    console.log('⚠️  ESLint not configured, skipping linting');
    testResults.warnings.push('ESLint not configured - consider adding for code quality');
  }
}
...
function checkDependencies() {
  ...
  try {
    ...
    if (heavyDeps.length > 0) {
      testResults.warnings.push(`Heavy dependencies detected: ${heavyDeps.join(', ')}`);
    }
    
    testResults.optimizations.push(`Dependency analysis completed: ${dependencies.length} prod deps`);
    return true;
  } catch (error) {
...
async function runProductionBuildTest() {
  ...
  const buildResult = runCommand('npx expo export --platform web', 'Web production build');
  ...
  if (testResults.errors.length > 0) {
    ...
    testResults.errors.forEach(error => console.log(`   - ${error.step}: ${error.error}`));
  }
