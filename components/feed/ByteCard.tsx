...
import { COLORS, SPACING, TYPOGRAPHY, BORDER_RADIUS, SHADOWS } from '../../shared/theme-constants';
...
export const ByteCard = memo<ByteCardProps>(({
  ...
  // Use real API interactions
  const { toggleLike, toggleBookmark, isLiking, isBookmarking } = useUserInteractions();
  ...
  const handleLike = useCallback(() => {
    ...
    // Call the real API (we'll need to get the post ID from the topic)
    // For now, we'll use the topic ID as a placeholder
    // TODO: Implement proper post ID mapping
    // toggleLike(parseInt(id));
  }, [onLike, likeAnimation, id]);
  ...
  const handleTeretPress = useCallback(() => onTeretPress?.(), [onTeretPress]);
  const handleAuthorPress = useCallback(() => onAuthorPress?.(), [onAuthorPress]);
      ...
      >
        ...
        <View style={styles.header}>
          <View style={styles.titleSection}>
            ...
            <View style={styles.timestampContainer}>
              <Clock size={12} weight="regular" color={colors.mutedForeground} />
          ...
          >
            <DotsThree size={20} weight="bold" color={colors.mutedForeground} />
          </TouchableOpacity>
        </View>
        ...
        {content && content !== 'No content available' && (
          <View style={styles.contentSection}>
            <Text style={[styles.content, { color: colors.secondary }]} numberOfLines={4}>
              {content}
            </Text>
          </View>
        )}
        ...
        <Animated.View style={{ transform: [{ scale: teretBadgeScale }] }}>
          ...
          >
            <View style={styles.teretBadgeContent}>
              <View style={[styles.teretIcon, { backgroundColor: colors.teretIconBg }]}>
                <Sparkle size={12} weight="bold" color={colors.teretIconColor} />
              </View>
        ...
        <View style={[styles.actionBar, { backgroundColor: colors.actionBarBg, borderTopColor: colors.border }]}>
          <Pressable
            onPress={handleLike}
          ...
          <Pressable
            ...
            hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}
          >
            <ChatCircle size={22} weight="regular" color={colors.mutedForeground} />
            {comments > 0 && (
