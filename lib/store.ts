...
const STORAGE_KEYS = {
  RSA_PRIVATE_KEY: 'rsa_private_pem',
...
export async function savePrivateKey(privateKeyPem: string): Promise<void> {
  try {
    await SecureStore.setItemAsync(STORAGE_KEYS.RSA_PRIVATE_KEY, privateKeyPem);
  } catch (error) {
...
export async function loadPrivateKey(): Promise<string | null> {
  try {
    return await SecureStore.getItemAsync(STORAGE_KEYS.RSA_PRIVATE_KEY);
  } catch (error) {
...
export async function saveUserApiKey(apiKey: string): Promise<void> {
  try {
    await SecureStore.setItemAsync(STORAGE_KEYS.USER_API_KEY, apiKey);
  } catch (error) {
...
export async function loadUserApiKey(): Promise<string | null> {
  try {
    return await SecureStore.getItemAsync(STORAGE_KEYS.USER_API_KEY);
  } catch (error) {
...
export async function loadClientId(): Promise<string | null> {
  try {
    return await SecureStore.getItemAsync(STORAGE_KEYS.CLIENT_ID);
  } catch (error) {
...
export async function clearAll(): Promise<void> {
  try {
    await Promise.all([
      SecureStore.deleteItemAsync(STORAGE_KEYS.RSA_PRIVATE_KEY),
      SecureStore.deleteItemAsync(STORAGE_KEYS.USER_API_KEY),
      SecureStore.deleteItemAsync(STORAGE_KEYS.CLIENT_ID),
    ]);
  } catch (error) {
