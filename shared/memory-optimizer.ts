...
class MemoryOptimizer {
  ...
  checkForLeaks() {
    ...
    if (this.snapshots.length >= 5) {
      ...
      const growthRate =
        (currentSnapshot.estimatedUsage - oldSnapshot.estimatedUsage) /
        oldSnapshot.estimatedUsage;

      if (growthRate > 0.2) {
        // 20% growth
        logger.warn(
          `Potential memory leak detected: ${(growthRate * 100).toFixed(1)}% growth in 5 snapshots`
        );
    ...
    if (currentSnapshot.componentCount > 50) {
      logger.warn(
        `High component count: ${currentSnapshot.componentCount} components tracked`
      );
    }
    ...
    if (currentSnapshot.listenerCount > 100) {
      logger.warn(
        `High listener count: ${currentSnapshot.listenerCount} listeners tracked`
      );
    }
    ...
    if (currentSnapshot.timerCount > 20) {
      logger.warn(
        `High timer count: ${currentSnapshot.timerCount} timers tracked`
      );
    }
  }
  ...
  unregisterComponent(id: string) {
    ...
    cleanup.cleanupFunctions.forEach((fn) => {
      try {
        ...
        logger.error(
          `Error in cleanup function for ${cleanup.componentName}:`,
          error
        );
      }
    });
    ...
    cleanup.listeners.forEach((listener) => {
      try {
        ...
        logger.error(
          `Error removing listener for ${cleanup.componentName}:`,
          error
        );
      }
    });
    ...
    logger.debug(
      `Component ${cleanup.componentName} cleaned up after ${lifetime}ms`
    );
  }
  ...
  cleanupInactiveComponents(maxAge: number = 300000) {
    // 5 minutes
    const now = Date.now();
    ...
    toCleanup.forEach((id) => {
      logger.warn(
        `Cleaning up inactive component: ${this.componentCleanups.get(id)?.componentName}`
      );
  ...
  getMemoryReport() {
    ...
    return {
      ...
      components: Array.from(this.componentCleanups.entries()).map(
        ([id, cleanup]) => ({
          id,
  ...
  reportMemoryState() {
    ...
    const heavyComponents = report.components
      ...
      .sort(
        (a, b) =>
          b.timerCount + b.listenerCount - (a.timerCount + a.listenerCount)
      );

    if (heavyComponents.length > 0) {
      logger.warn(
        'Components with high resource usage:',
        heavyComponents.slice(0, 5)
      );
    }
  }
